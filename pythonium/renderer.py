...
class GifRenderer:
    def __init__(self, galaxy, title=None, speed=200):
        ...
        self._frames = []
        self.available_players_colors = [
            (238, 48, 47, 256),
            (80, 168, 224, 256),
        ]
        self.players_colors = {
            p: self.available_players_colors.pop() for p in galaxy.known_races
        }
        ...
        self.frame_size = (
            self.galaxy.size[0] + self.margin,
    ...
    def galaxy2frame_coordinates(self, coordinates):
        return (
            coordinates[0] + 10,
            coordinates[1] + self.header - self.margin,
        )
    ...
    def render_frame(self, context):
        ...
        draw.text(
            (self.margin, self.frame_size[1] - 10),
    ...
    def render_title(self, draw):
        draw.text(
            (self.margin, self.margin), self.title, font=self._title_font
        )
    ...
    def render_score(self, draw, context):
        ...
        draw.text(
            (self.margin, self.margin + 90), "Planets", font=self._score_font
        )

        draw.text(
            (self.margin, self.margin + 130), "Ships", font=self._score_font
        )
        ...
        for i, player_score in enumerate(score):
            ...
            draw.text(
                (self.margin + (i + 1) * 170, self.margin + 50),
                player_score["player"],
                font=self._score_font,
                fill=color,
            )
            draw.text(
                (self.margin + (i + 1) * 170, self.margin + 90),
                str(player_score["planets"]),
                font=self._score_font,
            )
            draw.text(
                (self.margin + (i + 1) * 170, self.margin + 130),
    ...
    def render_planet(self, draw, planet):
        ...
        bounding_box = (
            (
                centroid[0] - self.plantet_radius,
                centroid[1] - self.plantet_radius,
            ),
            (
                centroid[0] + self.plantet_radius,
                centroid[1] + self.plantet_radius,
    ...
    def render_ship(self, draw, ship):
        ...
        draw.ellipse(bounding_box, fill=(0, 0, 0, 0), outline=color, width=3)
    ...
    def render_explosion(self, draw, explosion):
        ...
        radius = max(
            50, self.plantet_radius + int(explosion.ships_involved * 0.5)
        )
        draw.ellipse(
            (
